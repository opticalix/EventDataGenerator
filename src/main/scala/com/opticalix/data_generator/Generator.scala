...
object Generator {
  ...
  def runEventGenerator(args: Array[String]) = {
    ...
    try {
      ...
      println(s"source=$source(${JavaUtils.formatSize(new File(source).length())}), delayNano=$delayNano, mode=$mode, startTime=${Utils.formatTimeNewApi(startTime, "yyyy-MM-dd HH:mm:ss.SSS")}")


      for (line <- Source.fromFile(source).getLines()) {
        EventOutputProcessor.output(line)
